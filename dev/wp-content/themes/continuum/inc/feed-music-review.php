...
$terms = wp_get_object_terms($post->ID,'music_artist');
$artist_name = $terms[0]->name;
$artist_slug = $terms[0]->slug;
//get actor for current post
$terms = wp_get_object_terms($post->ID,'music_type');
...
if(!$test_loop->have_posts()) {	
	...
	if($feedsort=="genre") $feedsort="artist"; //use a different filter
}	

wp_reset_query();

$hideartist=false; //used for hiding artist link if no other posts in this artist
$artistargs=array( // get articles with this same artist
	'post_type' => 'con_music_reviews',
	'paged' => $paged,
	'post__not_in' => array($post->ID),
	'posts_per_page' => $con_feed_review_num,
...
$test_loop = new WP_Query($artistargs);
if(!$test_loop->have_posts()) {
	$hideartist=true; //there are no related posts	
	if($feedsort=="artist" && !$hidegenre) { //use a different filter
		$feedsort="genre"; 
	} elseif($feedsort=="artist") {
		$feedsort="type";
	}
}	
...
$hidetype=false; //used for hiding type link if no other posts in this type
$typeargs=array( // get articles with this same type
	'post_type' => 'con_music_reviews',
	'paged' => $paged,
	'post__not_in' => array($post->ID),
	'posts_per_page' => $con_feed_review_num,
...
$test_loop = new WP_Query($typeargs);
if(!$test_loop->have_posts()) {
	$hidetype=true; //there are no related posts
	if($feedsort=="type" && !$hidegenre) { //use a different filter
		$feedsort="genre"; 
	} elseif($feedsort=="type" && !$hideartist) {
		$feedsort="artist";
	}
}	
...
//find out which query to currently use
if($hidegenre && $hideartist && $hidetype) $feedsort="allreviews";
if($feedsort=="genre" && !$hidegenre) {
	$args=$genreargs;
} elseif($feedsort=="artist" && !$hideartist) {
	$args=$artistargs;	
} elseif($feedsort=="type" && !$hidetype) {
	$args=$typeargs;	
} else {	
	$args=array(
		'post__not_in' => array($post->ID),
    ...
    <div id="feed"<?php if(!$con_feed_review_social) { ?> class="hide-social"<?php } ?>>
        ...
        <?php if(!$hidegenre) { ?>
            ...
            <div class="sort">
                ...
                <a title="<?php _e( 'Show '.$genre_name.' music reviews', 'continuum' ); ?>" class="<?php if($feedsort=='genre') { ?>current <?php } ?>tooltip" href="<?php con_get_feed_link('genre') ?>"><?php _e( $genre_name.' Reviews', 'continuum' ); ?></a>
        ...
        <?php if(!$hideartist) { ?>
            ...
            <div class="sort">
                ...
                <a title="<?php _e( 'Show reviews for '.$artist_name, 'continuum' ); ?>" class="<?php if($feedsort=='artist') { ?>current <?php } ?>tooltip" href="<?php con_get_feed_link('artist') ?>"><?php _e( $artist_name.' Reviews', 'continuum' ); ?></a>
        ...
        <?php if(!$hidetype) { ?>
            ...
            <div class="sort">
                ...
                <a title="<?php _e( 'Show '.$type_name.' reviews', 'continuum' ); ?>" class="<?php if($feedsort=='type') { ?>current <?php } ?>tooltip" href="<?php con_get_feed_link('type') ?>"><?php _e( $type_name.' Reviews', 'continuum' ); ?></a>
        ...
        <div class="sort">
            ...
            <a title="<?php _e( 'Show all music reviews', 'continuum' ); ?>" class="<?php if($feedsort=='allreviews') { ?>current <?php } ?>tooltip" href="<?php con_get_feed_link('allreviews') ?>"><?php _e( 'All Music Reviews', 'continuum' ); ?></a>
			...
			<div class="feed-panel reviews<?php if ($postcount % 3 == 0) { ?> right-most<?php } ?>">
				...
				<div class="inner">
					...
					<div class="category">                        
						...
						$cat_link = get_term_link($cat_slug, 'music_genre');
						if($cat_name=="") {
