...
//get the category for current post
$terms = wp_get_object_terms($post->ID,'product_category');	
$cat_name = $terms[0]->name;	
$cat_slug = $terms[0]->slug;	
//get top level brand for current post
...
<?php // setup the query
$hidecategory=false; //used for hiding category link if no other posts in this category
$catargs=array( // get articles with this same category
...
$test_loop = new WP_Query($catargs);
if(!$test_loop->have_posts()) {	
	$hidecategory=true; //there are no related posts
	if($feedsort=="category") $feedsort="brand"; //use a different filter
}	

wp_reset_query();

$hidebrand=false; //used for hiding brand link if no other posts in this brand
$brandargs=array( // get articles with this same brand
	'post_type' => 'con_product_reviews',
	'paged' => $paged,
	'post__not_in' => array($post->ID),
	'posts_per_page' => $con_feed_review_num,
...
$test_loop = new WP_Query($brandargs);
if(!$test_loop->have_posts()) {
	$hidebrand=true; //there are no related posts
	if($feedsort=="brand" && !$hidecategory) { //use a different filter
		$feedsort="category"; 
	}
}	
...
if($hidecategory && $hidebrand) $feedsort="allreviews";
if($feedsort=="category" && !$hidecategory) {
	$args=$catargs;
} elseif($feedsort=="brand" && !$hidebrand) {
	$args=$brandargs;	
} else {	
	$args=array(
		'post__not_in' => array($post->ID),
    ...
    <div id="feed"<?php if(!$con_feed_review_social) { ?> class="hide-social"<?php } ?>>
        ...
        <?php if(!$hidecategory) { ?>
            ...
            <div class="sort">
                ...
                <a title="<?php _e( 'Show product reviews in '.$cat_name, 'continuum' ); ?>" class="<?php if($feedsort=='category') { ?>current <?php } ?>tooltip" href="<?php con_get_feed_link('category') ?>"><?php _e( 'More from '.$cat_name, 'continuum' ); ?></a>
        ...
        <?php if(!$hidebrand) { ?>
            ...
            <div class="sort">
                ...
                <a title="<?php _e( 'Show reviews for '.$brand_name.' products.', 'continuum' ); ?>" class="<?php if($feedsort=='brand') { ?>current <?php } ?>tooltip" href="<?php con_get_feed_link('brand') ?>"><?php _e( 'All '.$brand_name.' Reviews', 'continuum' ); ?></a>
        ...
        <div class="sort">
            ...
            <a title="<?php _e( 'Show all product reviews', 'continuum' ); ?>" class="<?php if($feedsort=='allreviews') { ?>current <?php } ?>tooltip" href="<?php con_get_feed_link('allreviews') ?>"><?php _e( 'All Product Reviews', 'continuum' ); ?></a>
			...
			<div class="feed-panel reviews<?php if ($postcount % 3 == 0) { ?> right-most<?php } ?>">
				...
				<div class="inner">
					...
					<div class="category">                        
						...
						$cat_name = $terms[0]->name;
						$cat_slug = $terms[0]->slug;
						$cat_link = get_term_link($cat_slug, 'product_category');
						if($cat_name=="") {
